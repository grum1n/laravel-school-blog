        ...
        <div class="w-full h-full bg-gray-900 bg-opacity-90">
            <div class="mx-auto w-full lg:w-4/5 px-4 py-28 mt-0 text-gray-900 leading-normal border-rounded flex items-center justify-center">
                <div x-data="{ open: true }" class="max-w-md w-full p-4 bg-white rounded-lg shadow-lg">
                    <h1 class="text-2xl font-semibold text-center text-gray-500 mt-2 mb-6">Login</h1>
                    <form action="{{ route('login') }}" method="POST">
                        ...
                        <button type="submit" x-on:click="open = ! open" x-show="open" class="w-32 bg-gradient-to-r from-indigo-800 to-indigo-600 text-white py-2 rounded-lg mx-auto block focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-800 mb-2">
                            Login
                        </button>
                    </form>
                    <div class="text-center">
                        <p class="text-sm"><a href="{{ route('register') }}" class="text-indigo-600">Register here</a></p>
                    </div>
                </div>
            </div>
        </div>     
